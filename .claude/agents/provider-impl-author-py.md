...
description: "Scaffolds a new omkit.providers.ProviderBase subclass for a given LLM vendor. Enforces ProviderDocument/ProviderMetric shapes, cost recording, sanitation, structured logging."
...
1. Inherit from `omkit.providers.ProviderBase` (see `omkit/providers/base.py`).
...
6. Use `omkit.http.build_tenant_client()` for outbound HTTP — never raw `httpx.AsyncClient` (tenant headers + tracing baked in).
7. Register itself via provider registry's register hook (`ProviderRegistry`) — caller wires this; provider exposes class.

## Files to create

- `omkit/providers/<vendor>.py` — provider class itself.
